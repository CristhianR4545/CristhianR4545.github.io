...
filterOpcions = document.querySelectorAll(".filter button"),
filterName = document.querySelector(".filter-info .name"),
filterValue = document.querySelector(".filter-info .value"),
filterSlider = document.querySelector(".slider input"),
rotateOpcions = document.querySelectorAll(".rotate button"),
...
rotateOpcions.forEach(option => {
    option.addEventListener("click", () => { // adding click event listener to all rotate/flip buttons
        if(option.id === "left"){
            rotate -= 90; // if click btn is left rotate, decrement rotate value by -90
        } else if(option.id === "right"){
            rotate += 90; // if click btn is right rotate, decrement rotate value by +90
...
const resetFilter = () => {
    ...
    filterOpcions[0].click(); // checking brightness btn, so the brightness selected by default
    applyFilters();
}
...
const saveImage = () => {
    ...
    ctx.scale(flipHorizontal, flipVertical); // flip canvas, horizonatally / vertically
